...
func _ready() -> void:
	...
	_Fav.icon = load("res://Assets/Icons/Filedialog/star.svg")#get_icon("fav_icon", "CustomFileDialog")
	_Back.icon = load("res://Assets/Icons/Filedialog/back_arrow.svg")#get_icon("back_icon", "CustomFileDialog")
	_Forward.icon = load("res://Assets/Icons/Filedialog/forward_arrow.svg")#get_icon("forward_icon", "CustomFileDialog")
	_AddFolder.icon = load("res://Assets/Icons/Filedialog/add_folder.svg")#get_icon("add_folder_icon", "CustomFileDialog")
	...
	if _Path.text == "" || _File.text == "":
		...
		match mode:
			Mode.MODE_OPEN_FILES:
				...
				for name in files:
					var fullpath = _get_fullpath(name.strip_edges())
					
					if !dir.file_exists(fullpath):
						_NothingSelected.dialog_text = tr("File not found: ") + name
						_NothingSelected.popup_centered()
						files_checked = false
						break
					else:
			...
			Mode.MODE_SAVE_FILE:
				...
				if dir.file_exists(fullpath):
					...
					emit_signal("on_file_selected", fullpath)
					hide()
...
func _on_override_confirmed() -> void:
	emit_signal("on_file_selected", _get_fullpath(_File.text.strip_edges()))
	hide()
